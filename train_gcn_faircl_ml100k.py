...
def train_semigcn(model, useremb, sens, n_users, args, samp, lr=0.001, num_epochs=300, batch_size=100, device='cpu'):
    sens = torch.tensor(sens).to(torch.long).to(device)
    ...
    CLloss = Constrastive_loss_limited_sen(n_users, args.tau, u_sens, samp, sample_ratio=0.1, lim_ratio=0.3)
    final_loss = 0.0
    # # 生成与向量相同大小的随机高斯噪声
    mean = 0
    std = 1
    ...
    for j in tqdm(range(num_epochs)):
        ...
        if j != 0 and j % 20 == 0:
            ...
            pca_visual(test_uemb, test_sens)
        ...
        # # 使用PCA进行降维
        # pca = PCA(n_components=2)
        # reduced_data = pca.fit_transform(e_su_np)
        #
        # # 绘制可视化图像
        # colors = ['r', 'b']  # 标签为0的用红色，标签为1的用蓝色
        # for i in range(len(e_su_np)):
        #     plt.scatter(reduced_data[i, 0], reduced_data[i, 1], c=colors[sens[i]])
        #
        # plt.xlabel('Component 1')
        # plt.ylabel('Component 2')
        # plt.show()
        # pca = PCA(n_components=2)
        # vectors_2d = pca.fit_transform(e_su_np)
        #
        # # 绘制散点图
        # plt.scatter(vectors_2d[:, 0], vectors_2d[:, 1])
        # plt.xlabel('Component 1')
        # plt.ylabel('Component 2')
        # plt.title('High-dimensional Vectors Visualization')
        # plt.show()
...
def train_unify_mi(sens_enc, inter_enc, club, dataset, u_sens,
    ...
    for epoch in range(args.num_epochs):
        ...
        auc_one, auc_res = pc_gender_train_ml100k.clf_gender_all_pre('auc', epoch, t_user_emb.detach().cpu().numpy(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='./data/ml-100k/process/process.pkl')
    ...
    parser.add_argument('--pretrain_path', type=str, default='param/ml100k_gcn_base.pth')
    ...
    u_sens = u_age
    #pretrain_anchor(n_users, n_items, u_sens, train_u2i, args)
    #导入预训练的anchor
    ...
    pretrain_model = torch.load(args.pretrain_path, map_location=torch.device('cpu'))
    pretrain_useremb, pretrain_itememb = pretrain_model.forward()
    pretrain_useremb1 = pretrain_useremb.detach().cpu().numpy()
    su = Collaborative_signal_completion(pretrain_useremb1, u_sens, 100, args, lim_ratio=0.3).get_su()
    samp = 0.8 * su + 0.2 * samp
    #samp = samp.detach().cpu().numpy()
    # samp=np.load('param/samp.npy',allow_pickle=True)
